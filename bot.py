"""
Smart AI Telegram Bot v3.2 - Complete Bot Handler (FIXED STARTUP)
Features: Multi-modal AI, RAG, Vision, Web Search, FAST STARTUP
Fixed: Removed blocking operations, added timeout, better error handling
"""
...
async def verify_user(user_id: int) -> bool:
    """Verify if user has joined required channels and groups (with timeout)"""
    try:
        ...
        logger.warning(f"Verification timeout for user {user_id}")
        return False
    except Exception as e:
...
async def start_handler(event: Message):
    ...
    try:
        ...
        await event.respond("⚠️ Terjadi kesalahan. Coba lagi dengan /start")
...
---
🌐 *GPT-5 v3.2 with Web Search* | @durov9369"""
...
---
🌐 *GPT-5 v3.2* | @durov9369"""
        
        await event.respond(stats_text)
    
    except Exception as e:
        logger.error(f"Error in stats handler: {e}")
        await event.respond("⚠️ Gagal mengambil statistik. Coba lagi.")
...
async def clear_handler(event: Message):
    ...
    try:
        ...
        await event.respond("⚠️ Gagal menghapus history. Coba lagi.")
...
**Developer:** @durov9369
**Version:** 3.2 Multi-Modal + Web Search
...
# ================== MAIN FUNCTION (FIXED FAST STARTUP) ==================

async def main():
    """Main bot startup with FAST initialization"""
    global bot, db, ai
    
    logger.info("🚀 Starting Smart AI Bot v3.2 (FAST STARTUP)...")
    
    try:
        # Initialize bot client with connection timeout
        ...
        logger.info("🌐 GPT-5 v3.2 with Web Search by @durov9369 - Ready to serve!")
        
        # Keep running
        await bot.run_until_disconnected()
    
    except asyncio.TimeoutError:
        logger.error("❌ Startup timeout! Check your internet connection and credentials.")
        raise
