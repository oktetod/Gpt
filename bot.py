...
class PollinationsAI:
    ...
    async def detect_intent(self, message: str, has_photo: bool = False) -> Dict:
        ...
        if any(word in message_lower for word in ['code', 'program', 'function', 'script', 'debug', 'bug', 'kode']):
            return {
                'type': 'chat',
                'model': 'qwen-coder'
            }
        ...
        if any(word in message_lower for word in ['analyze', 'think', 'reason', 'explain', 'why', 'analisa', 'jelaskan']):
            return {
                'type': 'chat',
                'model': 'deepseek-r1'
            }
        ...
        if any(word in message_lower for word in ['search', 'find', 'latest', 'news', 'cari', 'terbaru']):
            return {
                'type': 'chat',
                'model': 'gemini-search'
            }
    ...
    def register_handlers(self):
        ...
        async def start_handler(event):
            ...
            await event.respond(
                ...
                f"• Kirim foto + instruksi untuk edit\n"
        ...
        async def message_handler(event):
            ...
            if has_photo:
                ...
                # Upload to temporary storage or use Telegram file URL
                # For now, we'll handle it differently
                await event.respond("📸 Foto diterima! Sedang memproses...")
            ...
            if intent['type'] == 'audio':
                ...
                await self.handle_chat(event, intent, message_text)
        ...
        async def handle_image_transform(self, event, intent, image_url):
            ...
            if not image_url:
                await event.respond("❌ Silakan kirim foto dengan caption instruksi edit.")
                return
            
            prompt = intent['prompt']
            model = intent['model']
            
            await event.respond(f"✏️ Transforming image...\n💡 Instruction: {prompt}")
        ...
        async def handle_chat(self, event, intent, message_text):
