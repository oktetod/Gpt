"""
Smart AI Telegram Bot v3.1 - Complete Bot Handler (WITH WEB SEARCH)
Features: Multi-modal AI, RAG, Vision, Web Search, 4096 character limit SOLVED
"""
...
async def handle_message(event: Message):
    ...
    try:
        ...
        # Process with AI (includes web search if needed)
        ...
        if result['type'] == 'image':
            ...
            # Check web search rate limit if used
            if result.get('web_search_used'):
                if not await db.check_rate_limit(user_id, 'web_search', MAX_WEB_SEARCHES_PER_HOUR, 60):
                    await event.respond("⚠️ Batas web search tercapai. Coba lagi nanti.")
                    return
            
            # Text response (with smart split for 4096 limit)
            await send_long_message(event, result['content'])
...
async def main():
    """Main bot startup"""
    logger.info("🚀 Starting Smart AI Bot v3.1 (WITH WEB SEARCH)...")
